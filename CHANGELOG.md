...
- Credit preflight for `smartprospect_fetch_contacts` that rejects — rather than
  silently reduces — a request exceeding the account's available credits or
  single-fetch limit.
...
- `include_full_records: false` de-identification option on every contact-returning
  tool.
